...
export default function TradeCalculator({flatOffers}: { flatOffers: FlatOffer[] }) {
    ...
    const npcs = useMemo(() => {
        ...
        for (const o of flatOffers) {
            if (!seen.has(o.npcId))
                seen.set(o.npcId, {id: o.npcId, name: o.npcName, iconUrl: o.npcIconUrl});
        }
        return Array.from(seen.values());
    }, [flatOffers]);

    const factionNpcNames = new Set<string>(FACTIONS.flatMap(f => [...f.npcNames]));
    ...
    const filtered = useMemo(() => flatOffers.filter(o => {
        const matchesNpc = selectedNpcIds.size === 0 || selectedNpcIds.has(o.npcId);
      ...
      {t('filterByNpc')}
        ...
        {t('others')}
                                ...
                                {standaloneNpcs.map(npc => (
                                    <button key={npc.id} onClick={() => toggleNpc(npc.id)}
                                        ...
                                        {npc.iconUrl && (
                                            ...
                                            <img src={npc.iconUrl} alt={npc.name} width={40} height={40}
                                                 style={{
                                                     imageRendering: 'pixelated', flexShrink: 0,
                                                     filter: (selectedNpcIds.size === 0 || selectedNpcIds.has(npc.id)) ? 'none' : 'grayscale(1)',
                    ...
                    {FACTIONS.map(faction => {
                        const fNpcs = npcs.filter(n => (faction.npcNames as readonly string[]).includes(n.name));
                        ...
                        return (
                            <div key={faction.label} style={{
                                display: 'flex', flexDirection: 'column', gap: '0.4rem',
                                border: `1px solid rgba(${faction.rgb}, ${someIn ? '0.45' : '0.18'})`,
                                borderRadius: '0.6rem',
                                padding: '0.45rem 0.6rem',
                                background: someIn ? `rgba(${faction.rgb}, 0.06)` : 'transparent',
            ...
            <div style={card}>
                ...
                <div style={{position: 'relative'}}>
                    ...
                    {showDropdown && filtered.length > 0 && (
                        <ul style={{
                            ...
                            {filtered.map(offer => (
                                <li key={offer.offerId} onClick={() => handleSelect(offer)} style={{
                                    ...
                                    {offer.iconUrl ? (
                                        // eslint-disable-next-line @next/next/no-img-element
                                        <img src={offer.iconUrl} alt={offer.name} width={32} height={32}
                                    ...
                                    <span
                                        style={{fontSize: '0.75rem', color: 'rgb(var(--muted))', whiteSpace: 'nowrap'}}>
                ...
                {selected && (
                    <div style={{
                        ...
                        {selected.iconUrl && (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={selected.iconUrl} alt={selected.name} width={40} height={40}
                                 style={{imageRendering: 'pixelated', flexShrink: 0}}/>
                        )}
                        <span>
                            ...
                            <tr key={entry.id} style={{borderBottom: '1px solid rgb(var(--border))'}}>
                                <td style={{padding: '0.4rem'}}>
                                    <div style={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                                        {entry.offer.iconUrl && (
                                            // eslint-disable-next-line @next/next/no-img-element
                                            <img src={entry.offer.iconUrl} alt={entry.offer.name} width={24} height={24}
                                                 style={{imageRendering: 'pixelated', flexShrink: 0}}/>
                                        )}
                                        {entry.offer.name}
                                    </div>
                                ...
                                <td style={{padding: '0.4rem'}}>
                                    <button onClick={() => setCart(p => p.filter(e => e.id !== entry.id))}
                                            style={{
                                                background: 'none',
                                                border: 'none',
                                                cursor: 'pointer',
